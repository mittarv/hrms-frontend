...
const PolicyTable = ({ isEdit, toggleOptions }) => {
  ...
  const { myHrmsAccess, policy, loading } = useSelector((state) => state.hrRepositoryReducer);
        ...
        <Table style={{ marginTop: "20px" }}>
          <TableHead>
            <TableRow>
              ...
              {(allToolsAccessDetails?.[selectedToolName] >= 900 || myHrmsAccess?.permissions?.some(perm => perm.name === "policyAdmin_view")) && (
                <>
                  <StyledTableCell style={TableHeaderStyle}>
                    Approved by
                  </StyledTableCell>
