...
const ImportantEditTable = ({ isEdit, toggleOptions }) => {
  ...
  const canViewAdmin = allToolsAccessDetails?.[selectedToolName] >= 900 || 
    myHrmsAccess?.permissions?.some(perm => perm.name === "ImportantLinkAdmin_view");
  ...
  const handleCreateNewRow = () => {
    if (!canCreate) {
      alert("You don't have permission to create important link");
      return;
    }
  ...
  const deleteSelectedPolicy = () => {
    if (!canDelete) {
      alert("You don't have permission to delete important link");
      return;
    }
  ...
  const saveChanges = () => {
    ...
    if (newLinks.length > 0 && !canCreate) {
      alert("You don't have permission to create important link");
      return;
    }
    if (updatedLinks.length > 0 && !canUpdate) {
      alert("You don't have permission to update important link");
      return;
    }
    if (deletedLinks.length > 0 && !canDelete) {
      alert("You don't have permission to delete important link");
      return;
    }
  ...
  return (
    <>
              ...
              tempImportantLink?.map((tool) => (
                ...
                >
                  <TableCell style={ToolNameStyle} component="th" scope="row">
                    <div className="important_link_tool_input_div">
                      <Checkbox
                        checked={
                          selectedLinks.includes(tool.id) ||
                      ...
                      {deletedLinks.includes(tool.id) &&
                      tool?.toolName !== "" ? (
                        <p className="text127 important_link_tool_input ">
                        ...
                        <textarea
                          ...
                          className={`text7 important_link_tool_input ${
                            tool.toolName ===
                            importantLink.find(
                              (thisTool) => thisTool.id === tool.id
                            )?.toolName
                              ? ""
                              : "important_link_edited_input_box"
                          }`}
                          type="text"
                          value={tool?.toolName}
                          onChange={(e) => {
                  ...
                  { (allToolsAccessDetails?.[selectedToolName] >= 900 || myHrmsAccess?.permissions?.some(perm => perm.name === "ImportantLink_admin_view")) && (
                    <>
                ...
                >
                  <TableCell style={ToolNameStyle} component="th" scope="row">
                    <div className="important_link_tool_input_div">
                      {deletedLinks.includes(tool.id) &&
                      tool?.toolName !== "" ? (
                        <p className="text127 important_link_tool_input ">{tool?.toolName}</p>
                      ) : (
                        <textarea
                          rows={1}
                          placeholder="enter tool name"
                  ...
                  <TableCell style={ToolNameStyle} component="th" scope="row">
                    <div className="important_link_tool_input_div">
                      {deletedLinks.includes(tool.id) &&
                      tool?.toolLink !== "" ? (
                        <p className="text127 important_link_tool_input ">{tool?.toolLink}</p>
                      ) : (
                        <textarea
                          rows={1}
                          placeholder="enter tool link (e.g., https://example.com)"
      ...
      {(canDelete || allToolsAccessDetails?.[selectedToolName] >= 900) && (selectedLinks.length !== 0 || deletedLinks.length !== 0) ? (
        <div className="important_link_deleteButtons_section">
          {selectedLinks.length !== 0 ? (
            <button className="important_link_delete_button" onClick={deleteSelectedPolicy}>
              Delete Selected
            </button>
          ) : (
            <button className="important_link_delete_button" disabled>
              Delete Selected
            </button>
          )}
        </div>
